...
/*   Updated: 2024/12/18 06:09:13 by tguerran         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_textures(t_data *data)
{
	...
	if (!data->textures)
		clean_exit(data, 5);
	data->textures[NORTH] = load_texture_to_buffer(data, data->texinfo.north, data->texinfo.size);
	data->textures[SOUTH] = load_texture_to_buffer(data, data->texinfo.south, data->texinfo.size);
	data->textures[EAST] = load_texture_to_buffer(data, data->texinfo.east, data->texinfo.size);
	data->textures[WEST] = load_texture_to_buffer(data, data->texinfo.west, data->texinfo.size);
}

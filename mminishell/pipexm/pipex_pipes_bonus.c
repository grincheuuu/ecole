...
void	ft_un(char **argv, char **env, t_listp **listp, t_pointer_p *pointer)
{
	char	**patch;
	int		t;
	int		file_fd;
...
void	ft_mid(char *argv, char **env, t_listp **listp, t_pointer_p *pointer)
{
	char	**patch;
	int		t;
...
int	ft_last(int argc, char **argv, char **env, t_listp **listp)
{
	...
	if ((*listp)->pid == 0)
	{
		...
		if (ft_error(file_fdfinal, "file_fdfinal") == 1)
			return (1);
		dup2((*listp)->before->pipe_fd[0], STDIN_FILENO);
		close((*listp)->before->pipe_fd[0]);

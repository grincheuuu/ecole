...
void CgiExecution::methodeType2(std::string &path)
{
    char **envp;
    ...
    envp[7] = strdup("SERVER_PORT=");
    envp[8] = strdup(("PATH_INFO=" + _pythonPath).c_str());
    envp[9] = strdup(("SCRIPT_NAME=http://localhost:8080/directory/youpi.bla"));
    envp[10] = strdup("REMOTE_ADDR=");
    std::cout << std::endl << Purple "env2 " << envp[9] << Reset_Color << std::endl;
    for(int i = 0; envp[i] != NULL; i++)
        printf("%s\n", envp[i]);
    std::cout << std::endl;
    executeCgi2(envp, path);
    ft_fre(envp);
}

void CgiExecution::executeCgi2(char **envp, std::string &path)
{
    int pipe_fd[2];
    int pipe_in[2];
    pid_t pid;
    ...
    if (pid == 0)
    {
        ...
        char **tab;
        tab = (char **)malloc(2 * sizeof(char*));
        tab[0] = ft_strdup("testers/ubuntu_cgi_tester");
        tab[1] = NULL;
        if (access(tab[0], X_OK) == -1)
        {
            std::cerr << "Erreur d'accès à script cgi : " << strerror(errno) << std::endl;

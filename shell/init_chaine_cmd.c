...
int ft_init_maillon_cmd(int argc, char **argv)
{
...
t_cmd	*ft_lstnew_cmd(char ***str)
{
    ...
    new_node->option = ft_strdup((*str)[1]);
    if (new_node->option == NULL)
	{
        ft_free_node_cmd(new_node);
		return (NULL);
	}
    new_node->type = ft_strdup((*str)[2]);
    if (new_node->type == NULL)
	{
		ft_free_node_cmd(new_node);
		return (NULL);
	}
    new_node->redir = ft_strdup((*str)[3]);
	if (new_node->redir == NULL)
	{
		ft_free_node_cmd(new_node);
		return (NULL);
	}

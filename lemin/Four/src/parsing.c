...
t_ant **init_ants(int nbr_ants)
{
	t_ant **ants;

	ants = malloc(sizeof(t_ant *) * (nbr_ants + 1));
	ants[nbr_ants] = NULL;
...
void 	parsing(t_lem_in *all)
{
	...
	t_list *tmp1 = parse.file;
	while (tmp1)
	{
		char *line;

		line = (char *)tmp1->content;
		ft_putstr_fd(line, 1);
		ft_putchar_fd('\n', 1);
		tmp1 = tmp1->next;
	}
	ft_lstclear(&parse.file, del_str);
	t_list *tmp = parse.rooms;
	while (tmp)
	{
	...
	all->ants = init_ants(all->nb_ants);
	ft_putchar_fd('\n', 1);
}

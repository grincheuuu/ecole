...
//    print_space();
    ...
    if (point->size <= (len_page - sizeof(t_block)))
    {
        ft_putstr_fd("\nlarge-3\n", 1);
        len = len_page;
...
void    *ft_first_allocate_block(t_block *temp, size_t len_zone, size_t *in_stock)
{
    ...
    if (size_restante >= (ssize_t)(sizeof(t_block) + 16))
    {
        ...
        allocate->occupe = 1;
        if (in_stock)
            *in_stock -= (size_allo_only + sizeof(t_block));
        write(1, "\nstart\n", 7);
        print_space();
...
void    *ft_split_block(t_block *temp, size_t len_zone, size_t *in_stock)
{
    ...
    {
        allocate = temp;
        reste = (t_block *)((char *)(temp) + sizeof(t_block) + size_allo_only);
        reste->size = size_restante - sizeof(t_block);
        reste->occupe = 0;
        reste->debut_page = 0;
        reste->next = allocate->next;
...
void    *ft_generate_large(t_block **start, size_t len)
{
    ...
    if ((*start) == NULL)
    {
        ...
        print_space();
        return (void *)((char *)(*start) + sizeof(t_block));
    }
    else
    {
        iter = (*start);
...
void    *malloc(size_t size)
{
    ...
    if (len_zone <= (len_page - ft_align(sizeof(t_block))))//tiny
    {
//        show_alloc_mem();
    ...
    else if (len_zone <= (8 * len_page - ft_align(sizeof(t_block))))
    {
    ...
    else
    {
        ...
        return (ft_generate_large(&stock.large, (len_zone + 16)));
    }
}
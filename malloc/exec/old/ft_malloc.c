...
/*
void    *ft_generate_pointer(t_block *liste, size_t len_zone)
{
        ft_putstr_fd("\ntiny\n", 1);
        if (liste == NULL)
        {
            liste = create_block_page(2048);
            if (liste == NULL)
                return (NULL);
            return (ft_first_allocate_block(liste, len_zone));
        }
        else
        {
            t_block     *temp = liste;
            if (liste->size < len_zone)
            {
                ...
                if (temp == NULL)
                    return (NULL);
                liste->size += 2048 - sizeof(t_block);
                return (ft_first_allocate_block(temp, len_zone, liste));
            }
            while (temp->next != NULL)
            {
                if (temp->occupe == 0 && temp->size >= len_zone && temp->debut_page != 1)
                    return (ft_split_block_in(temp, len_zone, temp->next, liste));
                temp = temp->next;
            }
            if (temp->occupe == 0 && temp->size >= len_zone && temp->debut_page != 1)
                return (ft_split_block_in(temp, len_zone, temp->next, liste));
